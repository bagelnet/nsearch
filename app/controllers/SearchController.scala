...
class SearchController @Inject() (ws: WSClient) extends Controller {
  ...
  private def getAPIParamsDefault: Map[String, String] = {
    ...
    "filters[startTime][gte]" -> "2016-04-01T00:00:00+09:00",
    "filters[lengthSeconds][gte]" -> "0",
    "filters[lengthSeconds][lte]" -> "100",
    "_sort" -> "viewCounter",
    "_context" -> "nsearch")
  }
  ...
  private def createAPIParams(message: Param): Map[String, String] = {
    ...
    params foreach {
      ...
      case ("filters[lengthSeconds][lte]", v) => params = params.updated("filters[lengthSeconds][lte]", getParamsLengthFilters(message.p1).getOrElse(v))
      case ("filters[lengthSeconds][gte]", v) => params = params.updated("filters[lengthSeconds][gte]", getParamsLengthFilters(message.p2).getOrElse(v))
